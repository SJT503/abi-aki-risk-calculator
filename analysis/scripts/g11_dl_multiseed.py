# -*- coding: utf-8 -*-
"""G11: DL 基准多 seed（R3 审稿 Must-Fix #3）
==========================================
n7 v4 张量构建逐字复刻；seeds = {13, 2026}（42 = 已发表 n7_v4 值，活读汇总）
每 seed × 3 架构：intval AUROC/AUPRC → 跨 seed mean±SD + range
输出：results/g11_dl_multiseed.json
"""
import json
import os
import sys
import warnings

import numpy as np
import pandas as pd

warnings.filterwarnings("ignore")
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from gate import gate

gate(__file__)

import torch
import torch.nn as nn
from sklearn.metrics import roc_auc_score, average_precision_score

R = "E:/TBI subtype/09_tbi_aki/results"
H = 48
DEVICE = "cpu"
MAX_HR = 200

F = pd.read_parquet(f"{R}/n5_abi_rolling_features.parquet")
vit = pd.read_parquet(f"{R}/n4_abi_vitals.parquet")
labs = pd.read_parquet(f"{R}/n4_abi_labs.parquet").dropna(subset=["hr"])
uo_rows = pd.read_parquet(f"{R}/n2_m4_abi_uo_rows.parquet")
import duckdb
con = duckdb.connect()
adm = con.execute(f"""
SELECT c.stay_id, c.subject_id, a.admittime, p.anchor_year
FROM read_parquet('{R}/n1_m4_abi_cohort.parquet') c
JOIN read_csv_auto('E:/TBI subtype/data/mimic-iv-3.1/hosp/admissions.csv.gz') a ON c.hadm_id=a.hadm_id
JOIN read_csv_auto('E:/TBI subtype/data/mimic-iv-3.1/hosp/patients.csv.gz') p ON c.subject_id=p.subject_id
""").df()
pat2 = pd.read_csv("E:/TBI subtype/data/mimic-iv-3.1/hosp/patients.csv.gz", usecols=["subject_id", "anchor_year_group"])
_g = pat2["anchor_year_group"].astype(str).str.extract(r"(\d{4})\D+(\d{4})")
pat2["group_mid"] = (_g[0].astype(int) + _g[1].astype(int)) / 2.0
coh = adm.merge(pat2[["subject_id", "group_mid"]], on="subject_id", how="left")
coh["year"] = (coh.group_mid + (coh.admittime.dt.year - coh.anchor_year)).round().astype(int)
F = F.merge(coh[["stay_id", "year"]], on="stay_id", how="left")

TS_VARS = ["hr_rate", "sbp", "spo2", "temp_c", "rr", "cr", "na", "k", "glu", "bun", "hco3", "uo"]
NV_TS = len(TS_VARS)


def vec_hourly(df, hr_col, val_col, agg="last"):
    df = df.copy()
    df["_bin"] = np.clip(df[hr_col].to_numpy(float).astype(int), 0, MAX_HR - 1)
    df["_valid"] = (df[hr_col] >= 0) & (df[hr_col] < MAX_HR) & df[val_col].notna()
    df = df[df._valid].sort_values([hr_col], kind="mergesort")
    out = {}
    for sid, gg in df.groupby("stay_id", sort=False):
        if agg == "last":
            binned = gg.groupby("_bin")[val_col].last()
        else:
            binned = gg.groupby("_bin")[val_col].sum()
        arr = np.zeros(MAX_HR, np.float32)
        msk = np.zeros(MAX_HR, np.float32)
        arr[binned.index.to_numpy()] = binned.values.astype(np.float32)
        msk[binned.index.to_numpy()] = 1.0
        out[sid] = (arr, msk)
    return out


temp_c_native = vit[vit.itemid == 226329][["stay_id", "hr", "valuenum"]]
temp_f_conv = vit[vit.itemid == 223762].copy()
temp_f_conv["valuenum"] = (temp_f_conv.valuenum - 32) / 1.8
temp_f_conv = temp_f_conv[["stay_id", "hr", "valuenum"]]
temp_merged = pd.concat([temp_c_native, temp_f_conv], ignore_index=True)

VIT_MAP = {220045: "hr_rate", 220179: "sbp", 220277: "spo2", 220210: "rr"}
LAB_MAP = {50912: "cr", 50983: "na", 50971: "k", 50931: "glu", 51006: "bun", 50882: "hco3"}

hourly = {}
for iid, nm in VIT_MAP.items():
    hourly[nm] = vec_hourly(vit[vit.itemid == iid], "hr", "valuenum")
hourly["temp_c"] = vec_hourly(temp_merged, "hr", "valuenum")
for iid, nm in LAB_MAP.items():
    hourly[nm] = vec_hourly(labs[labs.itemid == iid], "hr", "valuenum")
uo_c = uo_rows.copy()
uo_c["_bin"] = np.clip(uo_c.hr_from_icu.to_numpy(float).astype(int), 0, MAX_HR - 1)
uo_c["_valid"] = (uo_c.hr_from_icu >= 0) & (uo_c.hr_from_icu < MAX_HR) & uo_c.uo_signed.notna()
uo_hourly = {}
for sid, gg in uo_c[uo_c._valid].groupby("stay_id", sort=False):
    s = gg.groupby("_bin").uo_signed.sum()
    arr = np.zeros(MAX_HR, np.float32)
    msk = np.zeros(MAX_HR, np.float32)
    arr[s.index.to_numpy()] = s.values.astype(np.float32)
    msk[s.index.to_numpy()] = 1.0
    uo_hourly[sid] = (arr, msk)
hourly["uo"] = uo_hourly

print("构建张量...")
grid = F[["stay_id", "t_hr", "label", "year"]].reset_index(drop=True)
tr_m = (grid.year <= 2017).values
tu_m = ((grid.year >= 2018) & (grid.year <= 2019)).values
iv_m = ((grid.year >= 2020) & (grid.year <= 2022)).values
y = grid.label.values.astype(np.float32)
sids_all = grid.stay_id.values
ts_all = grid.t_hr.values

var_arrays = {}
for var in TS_VARS:
    tbl = hourly.get(var, {})
    default = (np.zeros(MAX_HR, np.float32), np.zeros(MAX_HR, np.float32))
    arrs = np.zeros((len(grid), MAX_HR), np.float32)
    msks = np.zeros((len(grid), MAX_HR), np.float32)
    for i, sid in enumerate(sids_all):
        a, m = tbl.get(sid, default)
        arrs[i] = a
        msks[i] = m
    var_arrays[var] = (arrs, msks)


def build_batch(mask):
    idx = np.where(mask)[0]
    Xs = np.zeros((len(idx), H, NV_TS * 2), np.float32)
    for k, i in enumerate(idx):
        t = int(ts_all[i])
        h0 = t - H
        h1 = t
        lo = max(0, h0)
        hi = min(MAX_HR, h1)
        if hi <= lo:
            continue
        for j, var in enumerate(TS_VARS):
            arrs, msks = var_arrays[var]
            Xs[k, lo - h0:hi - h0, j] = arrs[i, lo:hi]
            Xs[k, lo - h0:hi - h0, j + NV_TS] = msks[i, lo:hi]
    return Xs, y[idx]


Xtr, ytr = build_batch(tr_m)
Xtu, ytu = build_batch(tu_m)
Xiv, yiv = build_batch(iv_m)
mu = Xtr[:, :, :NV_TS][Xtr[:, :, :NV_TS] != 0].mean() if (Xtr[:, :, :NV_TS] != 0).any() else 0.0
sd = max(Xtr[:, :, :NV_TS][Xtr[:, :, :NV_TS] != 0].std() if (Xtr[:, :, :NV_TS] != 0).any() else 1.0, 1e-6)
for X in (Xtr, Xtu, Xiv):
    X[:, :, :NV_TS] = (X[:, :, :NV_TS] - mu) / sd
print(f"train={Xtr.shape}({int(ytr.sum())}+) select={Xtu.shape}({int(ytu.sum())}+) intval={Xiv.shape}({int(yiv.sum())}+)")
del var_arrays


class MaskedCNN(nn.Module):
    def __init__(self, n_in):
        super().__init__()
        self.c1 = nn.Conv1d(n_in, 64, 3, padding=1)
        self.b1 = nn.BatchNorm1d(64)
        self.c2 = nn.Conv1d(64, 128, 3, padding=1)
        self.b2 = nn.BatchNorm1d(128)
        self.c3 = nn.Conv1d(128, 64, 3, padding=1)
        self.b3 = nn.BatchNorm1d(64)
        self.res = nn.Conv1d(n_in, 64, 1)
        self.pool = nn.AdaptiveMaxPool1d(1)
        self.drop = nn.Dropout(0.3)
        self.fc = nn.Linear(64, 1)

    def forward(self, x):
        x = x.permute(0, 2, 1)
        r = self.res(x)
        h = torch.relu(self.b1(self.c1(x)))
        h = torch.relu(self.b2(self.c2(h)))
        h = torch.relu(self.b3(self.c3(h))) + r
        return self.fc(self.drop(self.pool(h).squeeze(-1))).squeeze(-1)


class LSTMAttention(nn.Module):
    def __init__(self, n_in):
        super().__init__()
        self.lstm = nn.LSTM(n_in, 64, num_layers=3, batch_first=True, dropout=0.2)
        self.attn = nn.Linear(64, 1)
        self.drop = nn.Dropout(0.3)
        self.fc = nn.Linear(64, 1)

    def forward(self, x):
        o, _ = self.lstm(x)
        a = torch.softmax(self.attn(o), dim=1)
        return self.fc(self.drop((a * o).sum(dim=1))).squeeze(-1)


class MiniTransformer(nn.Module):
    def __init__(self, n_in):
        super().__init__()
        self.embed = nn.Linear(n_in, 64)
        self.pos = nn.Parameter(torch.randn(1, H, 64) * 0.02)
        self.cls = nn.Parameter(torch.randn(1, 1, 64) * 0.02)
        layer = nn.TransformerEncoderLayer(d_model=64, nhead=4, dim_feedforward=128,
                                           dropout=0.1, batch_first=True)
        self.encoder = nn.TransformerEncoder(layer, num_layers=2)
        self.drop = nn.Dropout(0.3)
        self.fc = nn.Linear(64, 1)

    def forward(self, x):
        e = self.embed(x) + self.pos
        cls = self.cls.expand(x.size(0), -1, -1)
        h = self.encoder(torch.cat([cls, e], dim=1))
        return self.fc(self.drop(h[:, 0])).squeeze(-1)


def run_once(cls, tag, seed):
    torch.manual_seed(seed)
    np.random.seed(seed)
    model = cls(NV_TS * 2).to(DEVICE)
    opt = torch.optim.AdamW(model.parameters(), lr=1e-4)
    pos_w = torch.tensor([(ytr == 0).sum() / max((ytr == 1).sum(), 1)], dtype=torch.float32)
    lossf = nn.BCEWithLogitsLoss(pos_weight=pos_w)
    bs = 32
    best_val = -1
    best_state = None
    patience = 0
    for ep in range(50):
        model.train()
        perm = np.random.permutation(len(ytr))
        for k in range(0, len(perm), bs):
            b = perm[k:k + bs]
            xb = torch.tensor(Xtr[b])
            yb = torch.tensor(ytr[b])
            opt.zero_grad()
            lossf(model(xb), yb).backward()
            opt.step()
        model.eval()
        with torch.no_grad():
            p_tu = torch.sigmoid(model(torch.tensor(Xtu))).numpy()
        val = roc_auc_score(ytu, p_tu)
        if val > best_val:
            best_val, patience = val, 0
            best_state = {k: v.clone() for k, v in model.state_dict().items()}
        else:
            patience += 1
            if patience >= 10:
                break
    model.load_state_dict(best_state)
    model.eval()
    with torch.no_grad():
        p_iv = torch.sigmoid(model(torch.tensor(Xiv))).numpy()
    return {"seed": seed,
            "best_select_auc": round(float(best_val), 4),
            "intval_auroc": round(float(roc_auc_score(yiv, p_iv)), 4),
            "auprc": round(float(average_precision_score(yiv, p_iv)), 4)}


SEEDS = [13, 2026]
results = {tag: [] for tag in ["masked_cnn", "lstm_attention", "mini_transformer"]}
for cls, tag in [(MaskedCNN, "masked_cnn"), (LSTMAttention, "lstm_attention"),
                 (MiniTransformer, "mini_transformer")]:
    for seed in SEEDS:
        r = run_once(cls, tag, seed)
        results[tag].append(r)
        print(f"{tag} seed={seed}: intval AUROC {r['intval_auroc']} AUPRC {r['auprc']} (ep best select {r['best_select_auc']})", flush=True)

# 汇总：并入 seed 42（n7 v4 已发表值，活读）
n7 = json.load(open(f"{R}/n7_deep_models.json", encoding="utf-8"))
agg = {}
for tag in results:
    vals = [x["intval_auroc"] for x in results[tag]] + [n7[tag]["intval_auroc"]]
    aup = [x["auprc"] for x in results[tag]] + [n7[tag]["auprc"]]
    agg[tag] = {"seeds": [13, 42, 2026],
                "intval_auroc_by_seed": {str(x["seed"]): x["intval_auroc"] for x in results[tag]},
                "seed42_from_n7": n7[tag]["intval_auroc"],
                "mean": round(float(np.mean(vals)), 4),
                "sd": round(float(np.std(vals, ddof=1)), 4),
                "range": [round(float(min(vals)), 4), round(float(max(vals)), 4)],
                "auprc_mean": round(float(np.mean(aup)), 4)}

out = {"probe": "G11 DL multi-seed (reviewer Must-Fix 3)", "date": "2026-09-23",
       "design": "seeds {13,42,2026} × 3 architectures; seed 42 = published n7_v4 values (verbatim)",
       "architectures": agg,
       "gbm_reference": 0.7369}
with open(f"{R}/g11_dl_multiseed.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=1)
print(json.dumps(agg, indent=1))
print("→ results/g11_dl_multiseed.json")
