# -*- coding: utf-8 -*-
"""N7 v4: 修复三 bug（UO sum + best_state + 温度合并）后重跑三模型"""
...
def vec_hourly(df, hr_col, val_col, agg="last"):
    """agg='last' for point measurements, 'sum' for volumetric (UO)"""
    df = df.copy()
...
hourly["temp_c"] = vec_hourly(temp_merged, "hr", "valuenum")  # 合并温度
...
for sid, gg in uo_c[uo_c._valid].groupby("stay_id", sort=False):
    s = gg.groupby("_bin").uo_signed.sum()  # ← 修复：sum 不是 last
    arr = np.zeros(MAX_HR, np.float32); msk = np.zeros(MAX_HR, np.float32)
...
for var in TS_VARS:
    print(f"  {var}: {len(hourly.get(var, {}))} stays")
...
def run(cls, tag):
    ...
    for ep in range(50):
        ...
        print(f"  ep{ep}:{val:.4f}",end="",flush=True)
        if val>best_val:
            best_val,patience=val,0
            best_state={k:v.clone() for k,v in model.state_dict().items()}  # ← 修复：保存最优
        else:
            patience+=1
            if patience>=10: print(" (stop)"); break
    print()
    model.load_state_dict(best_state)  # ← 修复：加载最优
    model.eval()
    with torch.no_grad():
    ...
    r={"tag":tag,"best_select_auc":round(float(best_val),4),
    ...
    print(f"  → {tag}: AUROC {r['intval_auroc']} {r['ci']} | AUPRC {r['auprc']} | 患者级 {r['patient_level']}")
    return r
...
out={"probe":"N7v4 ABI deep learning (3 bug fixes: UO sum, best_state, temp merge)","date":"2026-09-21",
     "fixes":["UO .last()→.sum()","best_state save+load","temp 226329+223762 merge"],
